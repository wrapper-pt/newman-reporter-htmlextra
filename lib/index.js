...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    newman.on('beforeDone', function () {
        ...
        const envDesc = this.summary.environment.values.filter(env => {
			return env.key == 'description';
		})
		.map(e => e.value);
	    
	var collectionDesc = this.summary.collection.name;
		if (this.summary.collection.description != null) {
			collectionDesc = this.summary.collection.description.content;
		}    
        ...
        this.exports.push({
            ...
            content: compiler({
                ...
                browserTitle: options.browserTitle || this.summary.collection.name,
                title: options.title || `${envDesc} - ${collectionDesc}`,
                titleSize: options.titleSize || 4,
